...
def applyTreeMasking(df, method_name):
    ...
    for i, (name, df_pixels) in enumerate(df.groupby("id_GEE")):
        ...
        df.loc[df.id_GEE == name, method_name] = newColumnsFromImage(df_pixels, maskTree1, True)
...
def apply_method_1_to_5(df, method_name):
    ...
    for i, (name, df_pixels) in enumerate(df.groupby("id_GEE")):
        ...
        for i in range(1,6):
            ...
            df.loc[df.id_GEE == name, method_cour_name] = newColumnsFromImage(df_pixels, cloud_score_persistence, False)
...
def apply_method_1_to_5_save(filename, method_name):
    """
    Arguments:
    ...
    nb_nan_values = df[df[method_name + "1"].isnull()][method_name + "1"].shape[0]
    ...
    # for i, (name, df_pixels) in enumerate(df_part.groupby("id_GEE")):
        ...
        boolean_vector_done = ~df_total[method_name + "1"].isnull()
        ...
        # for index, values in df_part.iterrows():
        #     for col in list(df_full.columns):
        #         column_index = cols_original.index(col)
                # print("col: ", col)
                # print("col index: ", column_index)
                # print("start index: ", start_index)
                # print("index: ",  index)
                # print("row index: ", start_index + index)
                # print("shape df_part: ", df_part.shape)
                # print("shape df_full: ", df_full.shape)
                # df_full.iloc[start_index + index, column_index] = values[col]
        ...
        #     column_index = cols_original.index(col)
            ...
            # print("shape df_part: ", df_part.shape)
            # print("shape df_full: ", df_full.shape)
            # df_full.iloc[start_index:start_index + df_pixels.shape[0], column_index] = df_part[col]
...
if __name__ == "__main__":
    ...
    apply_method_1_to_5_save(file_name, method_name)
